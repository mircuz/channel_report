...
The measures were carried out using the same code, compiled once for all the simulations, tested at $MPI~tasks$ variations.\par
Attention should be posed on the concept of $MPI~tasks$, $cores$ and $processors$ or $nodes$. The first identify the number of parallel processes of the simulation and is obtained as the product between the other two values:
\begin{equation*}
MPI~tasks = n~cores \times processors
\end{equation*}
 As we will see shortly the code exhibit sensible performances variation depending on the cores per nodes configuration. \par
